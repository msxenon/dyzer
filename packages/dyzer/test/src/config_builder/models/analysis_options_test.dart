...
import 'package:dyzer/src/config_builder/models/analysis_options.dart';
...
void main() {
  final collection = const AnalyzerUtils().createAnalysisContextCollection(
    ['test'],
  ...
  group('analysisOptionsFromFile constructs AnalysisOptions from', () {
    ...
    test('yaml file', () {
      ...
      expect(options.options, contains('dyzer'));
      expect(options.options['dyzer'], contains('metrics'));
      expect(
        (options.options['dyzer'] as Map<String, Object>)['metrics'],
        allOf(
      ...
      expect(options.options, contains('dyzer'));
      expect(options.options['dyzer'], contains('metrics-exclude'));
      expect(
        (options.options['dyzer'] as Map<String, Object>)['metrics-exclude'],
        containsAll(<String>['test/**', 'documentation/**']),
      );

      expect(options.options['dyzer'], contains('rules'));
      expect(
        (options.options['dyzer'] as Map<String, Object>)['rules'],
        allOf(
